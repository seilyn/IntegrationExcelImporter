...
namespace IntegrationExcelImporter.Core.DataAccess
{
    public class ExcelManager
    {
        ...
        public bool WriteMergedDataToNewSheet(ObservableCollection<Plan> mergedPlans, string outputFilePath)
        {
            ...
            try
            {
                ...
                using (var package = new ExcelPackage(new FileInfo(outputFilePath)))
                {
                    ...
                    List<Plan> sortedPlans = mergedPlans.ToList();
                    sortedPlans.Sort(ComparePlans);

                    // Clear the original ObservableCollection and add sorted items back
                    mergedPlans.Clear();
                    ...
                    for (int i = 0; i < mergedPlans.Count; i++)
                    {
                        ...
                        worksheet.Cells[i + 5, 9].Value = plan.Name;
                        worksheet.Cells[i + 5, 10].Value = plan.TuitionFee;
                        worksheet.Cells[i + 5, 11].Value = plan.TravelFee;
                        worksheet.Cells[i + 5, 12].Value = plan.SumOfFee;
                        worksheet.Cells[i + 5, 13].Value = plan.Janaury;
                        worksheet.Cells[i + 5, 14].Value = plan.Faburary;
                        worksheet.Cells[i + 5, 15].Value = plan.March;
                        worksheet.Cells[i + 5, 16].Value = plan.April;
                        worksheet.Cells[i + 5, 17].Value = plan.May;
                        worksheet.Cells[i + 5, 18].Value = plan.June;
                        worksheet.Cells[i + 5, 19].Value = plan.July;
                        worksheet.Cells[i + 5, 20].Value = plan.August;
                        worksheet.Cells[i + 5, 21].Value = plan.September;
                        worksheet.Cells[i + 5, 22].Value = plan.October;
                        worksheet.Cells[i + 5, 23].Value = plan.November;
                        worksheet.Cells[i + 5, 24].Value = plan.December;
                        worksheet.Cells[i + 5, 25].Value = plan.Spot;
                        worksheet.Cells[i + 5, 26].Value = plan.Reason;
                        worksheet.Cells[i + 5, 27].Value = plan.Planning;
                    ...
                    // 1월부터 12월까지 데이터를 만듭니다.

                    for (int month = 1; month <= 12; month++)
                    {

                        // 월에 해당하는 데이터를 필터링
                        List<Plan> monthlyPlans = mergedPlans.Where(p =>
                        {
                            return GetMonthValue(p, month) != "";

                        }).ToList();

                        // 월별 시트 생성
                        ExcelWorksheet sheets = package.Workbook.Worksheets.Add(month.ToString());
                        CreateHeader(sheets);

                        for (int i = 0; i < monthlyPlans.Count; i++)
                        {
                            var planTemp = monthlyPlans[i];
                            sheets.Cells[i + 5, 1].Value = planTemp.KindOfEducation;
                            sheets.Cells[i + 5, 2].Value = planTemp.EducationName;
                            sheets.Cells[i + 5, 3].Value = planTemp.EducationDay;
                            sheets.Cells[i + 5, 4].Value = planTemp.EducationTime;
                            sheets.Cells[i + 5, 5].Value = planTemp.EducationAgency;
                            sheets.Cells[i + 5, 6].Value = planTemp.Team;
                            sheets.Cells[i + 5, 7].Value = planTemp.Position;
                            sheets.Cells[i + 5, 9].Value = planTemp.Name;
                            sheets.Cells[i + 5, 10].Value = planTemp.TuitionFee;
                            sheets.Cells[i + 5, 11].Value = planTemp.TravelFee;
                            sheets.Cells[i + 5, 12].Value = planTemp.SumOfFee;

                            // 월별 데이터 추가
                            for (int j = 1; j <= 12; j++)
                            {
                                sheets.Cells[i + 5, 12 + j].Value = GetMonthValue(planTemp, j);
                            }

                            sheets.Cells[i + 5, 25].Value = planTemp.Spot;
                            sheets.Cells[i + 5, 26].Value = planTemp.Reason;
                            sheets.Cells[i + 5, 27].Value = planTemp.Planning;

                            for (int j = 12; j <= 23; j++)
                            {
                                sheets.Cells[i + 5, j].Style.WrapText = true;
                            }
                        }
                    }
        ...
        private int ComparePlans(Plan x, Plan y)
        {
            // 1월부터 12월까지 비교
            string[] monthsX = { x.Janaury, x.Faburary, x.March, x.April, x.May, x.June, x.July, x.August, x.September, x.October, x.November, x.December };
            string[] monthsY = { y.Janaury, y.Faburary, y.March, y.April, y.May, y.June, y.July, y.August, y.September, y.October, y.November, y.December };

            for (int i = 0; i < monthsX.Length; i++)
            {
                int compare = string.Compare(monthsY[i], monthsX[i]);
                if (compare != 0)
                {
                    return compare;
                }
        ...
        private object GetMonthValue(Plan plan, int month)
        {
            switch (month)
            {
                case 1:
                ...
                case 8:
                    return plan.August;
                case 9:
                    return plan.September;
                case 10:
                    return plan.October;
                case 11:
                    return plan.November;
                case 12:
                    return plan.December;
                default:
                    return null;
            }
        }
